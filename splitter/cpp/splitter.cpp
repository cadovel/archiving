...
PREPARE_LOGGING(splitter_i)

splitter_i::splitter_i(const char *uuid, const char *label) :
    splitter_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

splitter_i::~splitter_i()
{
}

void splitter_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void splitter_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &splitter_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &splitter_i::<callback method>)
        in the constructor.
        ...
        splitter_i::splitter_i(const char *uuid, const char *label) :
            splitter_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &splitter_i::scaleChanged);
            addPropertyListener(status, this, &splitter_i::statusChanged);
        }

        void splitter_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(splitter_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void splitter_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(splitter_i, "status changed");
        }
...
************************************************************************************************/
int splitter_i::serviceFunction()
{
    bulkio::InFloatStream inputStream = this->dataFloat->getCurrentStream();
    if (!inputStream) { // No streams are available
        return NOOP;
    }

    // Get the output stream, creating it if it doesn't exist yet
    bulkio::OutFloatStream outputStream_1 = this->dataFloat_1->getStream(inputStream.streamID());
    if (!outputStream_1) {
    	outputStream_1 = this->dataFloat_1->createStream(inputStream.sri());
    	const redhawk::PropertyMap &tmp = redhawk::PropertyMap::cast(inputStream.sri().keywords);
    	if (tmp.find("CHAN_RF") != tmp.end()) {
    		this->center_freq = tmp["CHAN_RF"].toDouble();
    	}
    	this->bandwidth = 1/inputStream.sri().xdelta;
    }
    bulkio::OutFloatStream outputStream_2 = this->dataFloat_2->getStream(inputStream.streamID());
    if (!outputStream_2) {
    	outputStream_2 = this->dataFloat_2->createStream(inputStream.sri());
    	const redhawk::PropertyMap &tmp = redhawk::PropertyMap::cast(inputStream.sri().keywords);
    	if (tmp.find("CHAN_RF") != tmp.end()) {
    		this->center_freq = tmp["CHAN_RF"].toDouble();
    	}
    ...
    if (block.sriChanged()) {
    	...
    	this->bandwidth = 1/inputStream.sri().xdelta;
        // Update output SRI
        outputStream_1.sri(block.sri());
        outputStream_2.sri(block.sri());
    }

    // Write to the output stream; outputData must not be modified after
    // this method call
    outputStream_1.write(block.buffer(), block.getStartTime());
    outputStream_2.write(block.buffer(), block.getStartTime());
