...
PREPARE_LOGGING(rfdatagen_i)

rfdatagen_i::rfdatagen_i(const char *uuid, const char *label) :
    rfdatagen_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

rfdatagen_i::~rfdatagen_i()
{
}

void rfdatagen_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void rfdatagen_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &rfdatagen_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &rfdatagen_i::<callback method>)
        in the constructor.
        ...
        //Add to rfdatagen.cpp
        rfdatagen_i::rfdatagen_i(const char *uuid, const char *label) :
            rfdatagen_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &rfdatagen_i::scaleChanged);
            addPropertyListener(status, this, &rfdatagen_i::statusChanged);
        }

        void rfdatagen_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(rfdatagen_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void rfdatagen_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(rfdatagen_i, "status changed");
        }
...
************************************************************************************************/
int rfdatagen_i::serviceFunction()
{
    std::string stream_id = "my_data";
	BULKIO::StreamSRI sri = bulkio::sri::create(stream_id);
	sri.xdelta = 1/this->sample_rate;
	sri.mode = 1;
	redhawk::PropertyMap &tmp = redhawk::PropertyMap::cast(sri.keywords);
	tmp["CHAN_RF"] = this->center_frequency;
	tmp["COL_RF"] = this->center_frequency;

	bulkio::OutFloatStream outputStream = this->dataFloat->getStream(stream_id);
    if (!outputStream) {
        outputStream = this->dataFloat->createStream(sri);
    }

    unsigned short pushes_per_second = 10;
    unsigned long number_samples = this->sample_rate/pushes_per_second;
    redhawk::buffer<std::complex<float> > outputData(number_samples);

    // Transform input data into output data
    for (size_t index = 0; index < number_samples; ++index) {
        outputData[index] = std::complex<float>(sin(2*3.14159*index/100), cos(2*3.14159*index/100));
    }

    // Write to the output stream; outputData must not be modified after
    // this method call
    outputStream.write(outputData, bulkio::time::utils::now());
    usleep(1e6/pushes_per_second);
